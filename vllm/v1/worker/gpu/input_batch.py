...
def _combine_sampled_and_draft_tokens_kernel(
    ...
    logits_indices_ptr,
    BLOCK_SIZE: tl.constexpr,
    NUM_SAMPLED_TOKENS: tl.constexpr = 1,
):
    batch_idx = tl.program_id(0)
    req_state_idx = tl.load(idx_mapping_ptr + batch_idx)
    ...
    num_draft_tokens = num_logits - NUM_SAMPLED_TOKENS
...
def combine_sampled_and_draft_tokens(
    ...
    assert num_sampled in (0, 1), f"num_sampled must be 0 or 1, got {num_sampled}"
    ...
    _combine_sampled_and_draft_tokens_kernel[(num_reqs,)](
        ...
        cu_num_logits,
        logits_indices,
        # NOTE(woosuk): Add num_sampled to ensure the block covers the
        # last sampled token in addition to all draft tokens.
        BLOCK_SIZE=triton.next_power_of_2(num_speculative_steps + num_sampled),
